...
class CreateModel:
    ...
    def __init__(self, cursor):
        ...
        # itemCategories
        rentitems_categories, rentitems_titles = self.items_categories(rentitems, 'rent')
        dealitems_categories, dealitems_titles = self.items_categories(dealitems, 'deal')

        rent_categories = [rentitems_categories[internal_id] for internal_id in range(1, n_items_rent+2)]
        deal_categories = [dealitems_categories[internal_id] for internal_id in range(1, n_items_deal+2)]
        ...
        sparse_rent
        self.user_indicator_features_rent
        self.full_rentitem_features
        n_items_rent

        sparse_deal
        self.user_indicator_features_deal
        self.full_dealitem_features
        n_items_deal
    ...
    def items_categories(self, items, rentordeal):
        items = items.values.tolist()
        if rentordeal == 'rent':
            lens_to_internal_item_ids = self.lens_to_internal_rentitem_ids
        else:
            lens_to_internal_item_ids = self.lens_to_internal_dealitem_ids
    ...
    def recommendation(self, userid, rentordeal):
        if rentordeal == 'rent':
            ...
            print(self.full_rentitem_features)
