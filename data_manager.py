...
class DataManager:
    ...
    def build(self, data, X = [], a = [], b = [], c = []):
        ...
        X = pd.concat([pd.DataFrame(X),
                       pd.concat([data[0], data[1], data[2], data[4], data[5]], axis=1)], axis=0)
        a = pd.concat([pd.DataFrame(a), data[1]], axis=0)
        b = pd.concat([pd.DataFrame(b), data[2]], axis=0)
        c = pd.concat([pd.DataFrame(c), data[3]], axis=0)
